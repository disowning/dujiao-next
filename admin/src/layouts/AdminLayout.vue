    ...
    <div class="flex min-h-screen">
      ...
      >
        <div class="py-6" :class="sidebarCollapsed ? 'px-2' : 'px-6'">
          ...
          <div v-if="sidebarCollapsed" class="text-lg font-semibold tracking-tight text-center">聚禧</div>
        </div>
        ...
        <div class="border-t border-border">
          <div v-if="!sidebarCollapsed" class="px-6 py-3 text-[11px] text-muted-foreground space-y-1">
            <p>© 2026 聚禧小铺. 保留所有权利 <span v-if="appVersion" class="text-muted-foreground/70">{{ appVersion }}</span></p>
            <a
              href="https://jx099.com"
              target="_blank"
              rel="noopener noreferrer"
              class="inline-flex items-center gap-1 underline-offset-2 hover:underline"
            >
              <span>https://jx099.com</span>
            </a>
          </div>
          <button
            type="button"
