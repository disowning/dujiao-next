...
const rebrandDisplayConfig = (value: any): any => {
    if (Array.isArray(value)) {
        return value.map(rebrandDisplayConfig)
    }
    if (value && typeof value === 'object') {
        return Object.fromEntries(
            Object.entries(value).map(([key, item]) => [key, rebrandDisplayConfig(item)]),
        )
    }
    if (typeof value !== 'string') {
        return value
    }
    const withoutOfficialLinks = value
        .replace(/(?:https?:\/\/)?github\.com\/dujiao-next(?:\/[\w./-]+)?/gi, SITE_URL)
        .replace(/https?:\/\/(?:www\.)?dujiao-next\.com(?:\/[\w./-]+)?/gi, SITE_URL)
    if (/^(?:https?:\/\/|\/)/i.test(withoutOfficialLinks)) {
        return withoutOfficialLinks
    }
    return withoutOfficialLinks.replace(/dujiao-next/gi, SITE_NAME)
}
    ...
    const loadConfig = async (force = false) => {
        ...
        try {
            ...
            config.value = {
                ...
                seo: rebrandDisplayConfig(loadedConfig.seo),
                legal: rebrandDisplayConfig(loadedConfig.legal),
                about: rebrandDisplayConfig(loadedConfig.about),
                footer_links: rebrandDisplayConfig(loadedConfig.footer_links),
                nav_config: rebrandDisplayConfig(loadedConfig.nav_config),
            }
            // 计算服务器与客户端的时间偏移量
